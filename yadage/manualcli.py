...
def connection_options(func):
    @click.option(
        ...
        "--metadir",
        default="yadagemeta",
        help="directory to store workflow metadata",
    )
    ...
    @click.option(
        ...
        multiple=True,
        default=None,
        help="options for the workflow controller",
    )
    ...
    @click.option(
        ...
        multiple=True,
        default=None,
        help="options for the workflow state models",
    )
...
@click.option(
    ...
    multiple=True,
    default=None,
    help="options for the workflow data state",
)
@click.option(
    "--metadir", default="yadagemeta", help="directory to store workflow metadata"
)
...
def init(
    dataarg, workflow, initfiles, modelsetup, dataopt, metadir, toplevel, parameter
):
...
def click_print_submittable_nodes(controller):
    ...
    for x in controller.adageobj.dag.nodes():
        ...
        if node.identifier in controller.submittable_nodes():
            click.secho(
                "node: {} ({}) part of stage {}".format(
                    node.name, node.identifier, "/".join([rule.offset, rule.rule.name])
                )
            )
...
def click_print_rule(rule, step_index, subrule_index, step_status):
    ...
    click.secho(
        "{}/{} [steps: ({})] [subflows: {}]".format(
            rule.offset,
            rule.rule.name,
            "/".join("{}: {}".format(k, v) for k, v in rule_stats.items()),
...
def click_print_applied_stages(controller):
    ...
    # print(step_status)
    for x in sorted(
        controller.adageobj.applied_rules,
        key=lambda r: "{}/{}".format(r.offset, r.rule.name),
    ):
...
def apply_stage(
    ...
    for n in name:
        ...
        if not rule:
            click.secho(
                "No such stage {}, pick one of the applicable below:".format(n),
                fg="red",
            )
            click_print_applicable_stages(controller)
            return
        ...
        if submit:
            _, s2r, _ = utils.rule_steps_indices(controller.adageobj)
            nodes_to_submit = [
                x for x in controller.submittable_nodes() if s2r[x] == rule.identifier
            ]
...
""".format(
            ...
            successful=click.style(
                str(controller.successful()),
                fg="green" if controller.successful() else "red",
            ),
            finished=click.style(
                str(controller.finished()),
                fg="green" if controller.finished() else "yellow",
            ),
            valid=click.style(
                str(controller.validate()),
                fg="green" if controller.validate() else "red",
            ),
...
def preview(
    ...
    click.secho(
        "Preview of Stage: # new rules: {} # new nodes {}".format(
            len(new_rules), len(new_nodes)
        )
    )
    for n in new_nodes:
        click.secho(
            '-> new node "{}" with {} upstream dependencies'.format(
                n["name"], len(n["parents"])
            )
        )
...
@click.option(
    ...
    "--nsteps",
    default=-1,
    help="number of steps to process. use -1 to for no limit (will run workflow to completion)",
)
...
@click.option(
    "-d", "--dataopts", multiple=True, default=None, help="ctrl opts for state provider"
)
...
def add(
    ...
    stages = (
        stages
        + workflow_loader.workflow(workflow, toplevel=toplevel, validate=True)["stages"]
    )
    controller.add_rules(
        stages, dataarg, offset=offset, groupname=groupname, dataopts=dataopts
    )
