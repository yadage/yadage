...
def main(workdir,analysis,initdata,toplevel,verbosity,loginterval,schemasource,parallel):
    logging.basicConfig(level = getattr(logging,verbosity))
    initdata = yaml.load(open(initdata)) if initdata else {}
    steering_api.run_workflow(workdir,analysis,initdata,toplevel,loginterval,schemadir = schemasource, nparallel = parallel)
