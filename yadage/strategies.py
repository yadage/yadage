...
def target(name,opts = None):
    ...
    def upstream(dag,target_identifier):
        ...
        return list(set(predecessors + [g for p in predecessors for g in upstream(dag,p)]))
    ...
    def submit_if_node_is_upstream(node, controller, idbased):
        ...
        log.debug('decide if submitting node: %s %s %s', node, node.identifier, target_node)
        if target_node:
