...
def load_provider(jsondata, deserialization_opts=None):
    ...
    if "state_provider" in deserialization_opts:
        ...
        if providerstring.startswith("py:"):
            return provider_deserialize_handlers["frompython_provider"](
                jsondata, deserialization_opts
            )
    if "YADAGE_STATEPROVIDER" in os.environ:
        return provider_deserialize_handlers["fromenv_provider"](
            jsondata, deserialization_opts
        )

    return provider_deserialize_handlers[jsondata["state_provider_type"]](
        jsondata, deserialization_opts
    )
...
def state_provider_from_string(dataarg, dataopts=None):
    ...
    raise RuntimeError(f"Unknown data type '{dataarg}, {dataopts}'.")
