    ...
    for stage, elements in scoped.items():
        ...
        for i, element in enumerate(elements):
            if "_nodeid" in element:
                ...
                additional = (
                    {"fixedsize": True, "height": 0.2, "width": 0.2}
...
def write_prov_graph(workdir, workflow, scope="", vizformat="pdf"):
    ...
    with open(
        "{}/yadage_workflow_instance.{}".format(workdir, vizformat), "w"
    ) as outfile:
        subprocess.call(
            shlex.split(
                "dot -T{} {}/yadage_workflow_instance.dot".format(vizformat, workdir)
            ),
            stdout=outfile,
        )
